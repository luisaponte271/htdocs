<?php
	...
	*/
	class User 
	{
		...
		{
			global $dbh, $lang;
			$getUsernameRef = $dbh->prepare("SELECT username,ip_register FROM users WHERE codigo = :codigo LIMIT 1");
			$getUsernameRef->bindParam(':codigo', $refUsername);
			...
			if ($getUsernameRef->RowCount() > 0)
			{
				...
				// {
				// 	echo 'ip base de datos->' . $getUsernameRefData['ip_register'];
				// 	echo 'ip logica->' .userIp(); 
				// 	echo 'ref_error';
				// }
				// else
				// {
																...
																if (self::refUser($_POST['referrer']) || empty($_POST['referrer']))
																{
																	...
																	{			
																		...
																		if ($config['hotelEmu'] == 'arcturus')
																		{
																			...
																			INSERT INTO
																			users
																			(codigo,username, password, rank, auth_ticket, motto, account_created, last_online, mail, look, ip_current, ip_register, credits)
																			VALUES
																			(
																			:codigo,
																			:username, 
																			...
																			$addNewUser->bindParam(':codigo', $codigo[0]['codigo'] );
																		...
																		else
																		{
																			...
																			INSERT INTO
																			users
																			(codigo, username, password, rank, auth_ticket, motto, account_created, last_online, mail, look, ip_last, ip_reg, credits, activity_points, vip_points)
																			VALUES
																			(
																			:codigo,
																			:username, 
																		...
																		{	
																			$getUserRef = $dbh->prepare("SELECT id,username, codigo FROM users WHERE codigo = :codigo LIMIT 1");
																			$getUserRef->bindParam(':codigo', $_POST['referrer']);
																			$getUserRef->execute();
		...
		Public static function generateCode() {
			global $dbh;
			$stmt = $dbh->prepare("SELECT LEFT(UUID(), 20) AS codigo"); 
				$stmt->execute(); 
